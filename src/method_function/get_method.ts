    ...
    if (pathname.startsWith("/api/")) {
        ...
        switch(api_path) {
            ...
            case "barang_kosong": {
                ...
                const stmt = db.prepare("SELECT nama_barang FROM barang WHERE stok_barang <= 0");
                const res = stmt.all();
                stmt.finalize();
            ...
            case "penjualan_item_tanggal": {
                ...
                const stmt = db.prepare("SELECT nama_barang, SUM(jumlah) AS jumlah FROM penjualan_item WHERE tanggal_key BETWEEN ? AND ? GROUP BY nama_barang");
            ...
            case "barang": {
                ...
                let res;
                if (isNaN(id) || !id) {
                    const stmt = db.prepare("SELECT b.id, b.nama_barang, b.stok_barang, b.kategori_barang_id, b.harga_modal, b.harga_jual, b.barcode_barang, b.created_ms, b.modified_ms, k.nama_kategori AS nama_kategori FROM barang b JOIN kategori_barang k ON b.kategori_barang_id = k.id");
                    res = stmt.all();
                    stmt.finalize();
                }
                else {
                    const stmt = db.prepare("SELECT b.id, b.nama_barang, b.stok_barang, b.kategori_barang_id, b.harga_modal, b.harga_jual, b.barcode_barang, b.created_ms, b.modified_ms, k.nama_kategori AS nama_kategori FROM barang b JOIN kategori_barang k ON b.kategori_barang_id = k.id WHERE b.id = ?");
                    res = stmt.get(id);
                    stmt.finalize();
                }
            ...
            case "kategori_barang": {
                ...
                let res;
                if (isNaN(id) || !id) {
                    const stmt = db.prepare("SELECT * FROM kategori_barang");
                    res = stmt.all();
                    stmt.finalize();
                }
                else {
                    const stmt = db.prepare("SELECT * FROM kategori_barang WHERE id = ?");
                    res = stmt.get(id);
                    stmt.finalize();
                }
            ...
            case "bak_list": { // barang assigned kategori (BAK) list
                ...
                const stmt = db.prepare("SELECT nama_barang, stok_barang, harga_jual FROM barang WHERE kategori_barang_id = ?");
            ...
            case "barang_masuk": {
                ...
                const stmt = db.prepare("SELECT bm.id, b.nama_barang, bm.deskripsi, bm.jumlah_barang FROM barang_masuk bm JOIN barang b ON b.id = bm.barang_id WHERE bm.tanggal_key = ?");
            ...
            case "penjualan": {
                ...
                if (isNaN(id) || !id) {
                    ...
                    const stmt = db.prepare("SELECT * FROM penjualan WHERE tanggal_key = ?");
                    res = stmt.all(tanggal_key);
                    stmt.finalize();
                }
                else {
                    const stmt = db.prepare("SELECT * FROM penjualan WHERE id = ?");
                    res = stmt.get(id);
                    stmt.finalize();
                }
            ...
            case "penjualan_item": {
                ...
                const stmt = db.prepare("SELECT jumlah, harga_jual, tanggal_key, created_ms, modified_ms, nama_barang FROM penjualan_item WHERE penjualan_id = ?");
            ...
            case "pengeluaran": {
                ...
                let res;
                if (!isNaN(id) && id) {
                    const stmt = db.prepare("SELECT * FROM pembukuan WHERE id = ? AND tipe = 1");
                    res = stmt.get(id);
                    stmt.finalize();
                }
                else {
                    if (isNaN(tanggal_key)) return new Response("Bad Request", {status: 400});
                    const stmt = db.prepare("SELECT * FROM pembukuan WHERE tanggal_key = ? AND tipe = 1");
                    res = stmt.all(tanggal_key);
                    stmt.finalize();
                }
            ...
            case "laporan": {
                ...
                let stmt = db.prepare("SELECT * FROM penjualan WHERE tanggal_key BETWEEN ? AND ?");
            ...
            case "user": { // get user information by id (administrator permission only)
                ...
                const stmt = db.prepare("SELECT username, full_name, role_id, profile_img, created_ms, modified_ms FROM users WHERE id = ?");
            ...
            case "users": { // get list of all users information (administrator permission only)
                ...
                const stmt = db.prepare("SELECT id, username, full_name, role_id, profile_img, created_ms, modified_ms FROM users");
                const res = stmt.all();
            ...
            case "uar_list": { // user assigned role (UAR) list
                ...
                const stmt = db.prepare("SELECT username, full_name FROM users WHERE role_id = ?");
                const res = stmt.all(id);
